"""
Experiment 082 — Final submission using exp080's ensemble with tuned
LightGBM (exp079's HPO result, learning_rate=0.0766 num_leaves=54
max_depth=5) replacing default params, alongside exp076's tuned XGBoost.
CV = 0.96747. Multi-seed bagging (3 seeds) as established.
"""
import time
import json
import pickle
...
def main():
    ...
    np.save("artifacts/pred_test_exp082_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp082_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp082_xgboost.npy", pred_xgb)

    print(f"Blend weights (from exp080): {weights}")
    ...
    old_sub = pd.read_csv("submission_v10.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v11.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v10.csv (prev): {diff.mean():.5f}")
    print(f"correlation with submission_v10.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
