"""
Experiment 047 — Get LightGBM and CatBoost OOF predictions with the new
final feature set (exp046 Variant B: 9 raw + entertainment_ratio +
social_ratio + gaming_ratio + workstudy_ratio), for the ensemble rebuild.
XGBoost's OOF is already available from exp046 (oof_exp046_variantB.npy).
"""
import time
import json
import numpy as np
...
print("=" * 70)
print("LightGBM + final feature set")
print("=" * 70)
lgbm_params = dict(
...
np.save("artifacts/oof_exp047_lgbm_final.npy", oof_lgbm)

print(f"\n{'=' * 70}\nCatBoost (tuned) + final feature set\n{'=' * 70}")
...
np.save("artifacts/oof_exp047_catboost_final.npy", oof_cb)

log_experiment({
    "exp_id": "exp047",
    "model": "LightGBM + CatBoost, final feature set (both ratios + decomposed components)",
    "features": "9 numeric + entertainment_ratio + social_ratio + gaming_ratio + workstudy_ratio",
    "preprocessing": "none (native NaN handling)",
    "hyperparams": "lgbm: exp001 untuned; catboost: exp023 tuned",
    ...
    "notes": "getting OOF for the ensemble rebuild with the final feature set",
    "conclusion": "TBD",
})
