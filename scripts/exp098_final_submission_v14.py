"""
Experiment 098 — Final submission using exp097's ensemble: expanded
consensus features + exact-value target encoding (the biggest single
win of the session). CV = 0.96856. Multi-seed bagging (3 seeds) as
established.
"""
import time
import json
...
def main():
    ...
    with open("artifacts/exp097_ensemble_weights.json") as f:
        weights = json.load(f)
    ...
    np.save("artifacts/pred_test_exp098_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp098_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp098_xgboost.npy", pred_xgb)

    print(f"Blend weights (from exp097): {weights}")
    ...
    old_sub = pd.read_csv("submission_v13.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v14.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v13.csv (prev): {diff.mean():.5f}")
    print(f"correlation with submission_v13.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
