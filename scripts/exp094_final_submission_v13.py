"""
Experiment 094 — Final submission using exp093's ensemble: expanded
consensus features (consensus-9 + 14 artifacts + 20 expanded-consensus),
all 3 models tuned. CV = 0.96785. Multi-seed bagging (3 seeds) as
established.
"""
import time
import json
...
def main():
    ...
    with open("artifacts/exp093_ensemble_weights.json") as f:
        weights = json.load(f)
    ...
    np.save("artifacts/pred_test_exp094_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp094_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp094_xgboost.npy", pred_xgb)

    print(f"Blend weights (from exp093): {weights}")
    ...
    old_sub = pd.read_csv("submission_v12.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v13.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v12.csv (prev): {diff.mean():.5f}")
    print(f"correlation with submission_v12.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
