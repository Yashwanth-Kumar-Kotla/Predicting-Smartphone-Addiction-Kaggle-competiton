"""
Experiment 081 — CatBoost is the last of the 3 ensemble members never
tuned on the consensus-9 feature set (still using exp023's params, which
were tuned on the ORIGINAL raw feature set before consensus features
existed). LightGBM HPO found +0.00045 (exp079), XGBoost found +0.00011
(exp075). Testing whether CatBoost has similar room.

Uses the cached consensus feature set (artifacts/consensus_data_cache.pkl)
to avoid redundant OpenFE rediscovery.
"""
import time
import json
import pickle
...
def main():
    ...
    if study.best_trial.number in trial_oofs:
        ...
        np.save("artifacts/oof_exp081_catboost_hpo_consensus.npy", best_oof)

    with open("artifacts/exp081_best_params.json", "w") as f:
