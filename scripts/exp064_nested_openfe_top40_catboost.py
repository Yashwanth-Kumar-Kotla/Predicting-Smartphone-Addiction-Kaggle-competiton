"""
Experiment 064 — exp061/062 found top-40 is the OpenFE sweet spot for
XGBoost (top-80 plateaued). Applying top-40 to CatBoost, which was still
on top-20 (exp058, +0.00105). Same leakage-safe nested-per-fold
methodology.
"""
import time
import json
...
def main():
    ...
    print(f"vs CatBoost top-20 nested (exp058, 0.96557): delta = {total_auc - 0.96557:+.5f}")
    print(f"vs CatBoost no-OpenFE baseline (0.96452): delta = {total_auc - 0.96452:+.5f}")

    np.save("artifacts/oof_exp064_nested_openfe_top40_catboost.npy", oof)

    log_experiment({
        "exp_id": "exp064",
        "model": "CatBoost (tuned) + nested OpenFE top 40 (leakage-safe)",
        ...
        "notes": "brings CatBoost to the validated top-40 sweet spot (was top-20 in exp058)",
        "conclusion": "TBD",
    })
