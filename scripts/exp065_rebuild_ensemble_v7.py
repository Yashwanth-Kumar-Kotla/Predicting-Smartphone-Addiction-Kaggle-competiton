"""
Experiment 065 — Rebuild the ensemble using top-40 nested-OpenFE OOF for
all 3 models (exp061 XGBoost 0.96720, exp063 LightGBM 0.96655, exp064
CatBoost 0.96674) -- the validated sweet spot, all leakage-safe.
"""
import numpy as np
...
print("Individual OOF AUCs (all with top-40 nested OpenFE):")
...
print(f"vs exp059 ensemble (top-20 OpenFE on all 3, 0.96622): delta = {best_auc - 0.96622:+.5f}")

np.save("artifacts/oof_exp065_ensemble_top40.npy", best_blend)
import json
with open("artifacts/exp065_ensemble_weights.json", "w") as f:
    json.dump(dict(zip(names, best_w.tolist())), f, indent=2)

from common import log_experiment
log_experiment({
    "exp_id": "exp065",
    "model": "Ensemble (LGBM+CatBoost+XGBoost, all 3 with top-40 nested OpenFE)",
    "features": "current best (13) + top 40 nested OpenFE, all 3 models",
    "preprocessing": "none",
    ...
    "runtime_sec": "~2",
    "notes": "full 3-model ensemble with top-40 OpenFE features on all models",
    "conclusion": "TBD",
})
