"""
Experiment 056 — Final submission using exp055's validated mixed
ensemble: LightGBM/CatBoost with current best features only, XGBoost with
current best features + OpenFE-discovered features (exp050's full-data
discovery, refit here on 100% of training data -- same principle as
deploying CV-tuned hyperparameters on a full-data refit). New ensemble
CV = 0.96615 (+0.00062 over the previous best submission's basis).

Multi-seed bagging (3 seeds) as established. OpenFE transform is
deterministic given the fitted feature list, so the same discovered
formulas apply cleanly to train (multiple seed subsplits) and test.
"""
import time
import json
import pickle
...
def main():
    ...
    X_openfe, Xte_openfe = transform(train[NUM_COLS].copy(), test[NUM_COLS].copy(), openfe_features[:20], n_jobs=8)
    openfe_cols = [c for c in X_openfe.columns if c not in NUM_COLS]
    for c in openfe_cols:
        train[c] = X_openfe[c].values
        test[c] = Xte_openfe[c].values

    xgb_feat_cols = current_best_feats + openfe_cols
    print(f"XGBoost feature count: {len(xgb_feat_cols)}  (LGBM/CatBoost: {len(current_best_feats)})")
    ...
    for seed in SEEDS:
        print(f"\n--- seed {seed} ---")
        Xtr_cb, Xval_cb, ytr, yval = train_test_split(
            train[current_best_feats], yfull, test_size=0.05, stratify=yfull, random_state=seed
        )
        val_idx = Xval_cb.index
        ...
        m.fit(Xtr_cb, ytr, eval_set=[(Xval_cb, yval)], eval_metric="auc",
              callbacks=[early_stopping(100, verbose=False), log_evaluation(0)])
        preds["lgbm"].append(m.predict_proba(test[current_best_feats])[:, 1])
        ...
        m.fit(Xtr_cb, ytr, eval_set=(Xval_cb, yval), use_best_model=True)
        preds["catboost"].append(m.predict_proba(test[current_best_feats])[:, 1])
        print(f"  best_iter={m.get_best_iteration()}")

        print("XGBoost (+ OpenFE features)...")
        Xtr_xgb = train[xgb_feat_cols].drop(index=val_idx)
        Xval_xgb = train[xgb_feat_cols].loc[val_idx]
        ytr_xgb = train[TARGET].drop(index=val_idx).values
        yval_xgb = train[TARGET].loc[val_idx].values
        xgb_params = dict(xgb_tuned, n_estimators=7000, tree_method="hist",
                           eval_metric="auc", early_stopping_rounds=100,
                           random_state=seed, n_jobs=-1)
        m = XGBClassifier(**xgb_params)
        m.fit(Xtr_xgb, ytr_xgb, eval_set=[(Xval_xgb, yval_xgb)], verbose=False)
        preds["xgboost"].append(m.predict_proba(test[xgb_feat_cols])[:, 1])
    ...
    np.save("artifacts/pred_test_exp056_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp056_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp056_xgboost.npy", pred_xgb)

    with open("artifacts/exp055_ensemble_weights.json") as f:
        weights = json.load(f)
    print(f"Blend weights (from exp055): {weights}")
    ...
    old_sub = pd.read_csv("submission_v5.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v6.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v5.csv (prev best): {diff.mean():.5f}")
    print(f"correlation with submission_v5.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
