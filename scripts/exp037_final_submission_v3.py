"""
Experiment 037 — Updated multi-seed submission with entertainment_ratio
added to all 3 models (exp034/035/036). New ensemble CV = 0.96511
(+0.00017 over the previous best, exp024/025's 0.96494).

Same proven approach as exp025: 3 seeds, small 5% stratified holdout per
seed for early-stopping determination, blend with exp036's re-optimized
weights (lgbm=0.115, catboost=0.267, xgboost=0.618).
"""
import time
import json
import numpy as np
...
with open("artifacts/exp036_ensemble_weights.json") as f:
    weights = json.load(f)
print(f"Blend weights (from exp036): {weights}")
...
print(f"submission_v3.csv written: {len(submission)} rows")
print(f"mean |change| vs submission_multiseed.csv (prev best): {diff.mean():.5f}")
