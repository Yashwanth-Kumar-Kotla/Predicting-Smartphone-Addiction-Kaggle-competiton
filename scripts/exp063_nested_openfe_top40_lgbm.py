"""
Experiment 063 — exp061/062 found top-40 OpenFE candidates is the sweet
spot for XGBoost (top-80 plateaued). Applying top-40 to LightGBM, which
was still on top-20 (exp057, +0.00090). Same leakage-safe nested-per-fold
methodology.
"""
import time
import numpy as np
...
def main():
    ...
    print(f"vs LightGBM top-20 nested (exp057, 0.96530): delta = {total_auc - 0.96530:+.5f}")
    print(f"vs LightGBM no-OpenFE baseline (0.96440): delta = {total_auc - 0.96440:+.5f}")

    np.save("artifacts/oof_exp063_nested_openfe_top40_lgbm.npy", oof)

    log_experiment({
        "exp_id": "exp063",
        "model": "LightGBM (untuned) + nested OpenFE top 40 (leakage-safe)",
        ...
        "notes": "tests top-40 sweet spot on LightGBM (was top-20 in exp057)",
        "conclusion": "TBD",
    })
