"""
Experiment 060 — Final submission using exp059's ensemble: nested-OpenFE
features on ALL THREE models. New ensemble CV = 0.96622.

OpenFE features refit on 100% of training data (exp050's discovery),
applied identically to all 3 models this time. Multi-seed bagging (3
seeds) as established.
"""
import time
import json
import pickle
...
def main():
    ...
    all_feat_cols = current_best_feats + openfe_cols
    print(f"Total feature count (all 3 models now): {len(all_feat_cols)}")
    ...
    np.save("artifacts/pred_test_exp060_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp060_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp060_xgboost.npy", pred_xgb)

    with open("artifacts/exp059_ensemble_weights.json") as f:
        weights = json.load(f)
    print(f"Blend weights (from exp059): {weights}")
    ...
    old_sub = pd.read_csv("submission_v6.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v7.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v6.csv (prev best): {diff.mean():.5f}")
    print(f"correlation with submission_v6.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
