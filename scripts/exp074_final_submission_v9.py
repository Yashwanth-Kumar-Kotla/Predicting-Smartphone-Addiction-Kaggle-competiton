"""
Experiment 074 — Final submission using exp073's consensus-9-feature
ensemble: 0.96734 CV, fully honest (validation methodology matches
deployment exactly for the first time -- no selection-bias mismatch).
...
def main():
    ...
    np.save("artifacts/pred_test_exp074_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp074_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp074_xgboost.npy", pred_xgb)

    with open("artifacts/exp073_ensemble_weights.json") as f:
        weights = json.load(f)
    print(f"Blend weights (from exp073): {weights}")
    ...
    old_sub = pd.read_csv("submission_v8.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v9.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v8.csv (prev best): {diff.mean():.5f}")
    print(f"correlation with submission_v8.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
