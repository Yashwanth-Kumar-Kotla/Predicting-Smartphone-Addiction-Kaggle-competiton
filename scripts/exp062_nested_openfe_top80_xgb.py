"""
Experiment 062 — exp061 showed top-40 OpenFE features beat top-20 by a
large, fully-honest margin (+0.00116) with LOWER fold variance, not
higher -- a strong positive scaling signal. Pushing further to top-80 to
find where returns diminish, same nested leakage-safe methodology.
"""
import time
import json
import pickle
...
def main():
    ...
    print(f"vs top-40 nested version (exp061, 0.96720): delta = {total_auc - 0.96720:+.5f}")
    print(f"vs current best no-OpenFE (0.96538): delta = {total_auc - 0.96538:+.5f}")

    np.save("artifacts/oof_exp062_nested_openfe_top80_xgb.npy", oof)
    with open("artifacts/exp062_fold_features.pkl", "wb") as f:
        pickle.dump(all_fold_features, f)

    log_experiment({
        "exp_id": "exp062",
        "model": "XGBoost (tuned) + nested OpenFE top 80 (leakage-safe)",
        "features": "current best (13) + top 80 OpenFE features discovered SEPARATELY per fold",
        ...
        "notes": "does an even larger OpenFE candidate pool (80 vs 40) find more real signal?",
        "conclusion": "TBD",
    })
