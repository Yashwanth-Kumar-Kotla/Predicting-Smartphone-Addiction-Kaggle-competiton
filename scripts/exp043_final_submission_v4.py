"""
Experiment 043 — Updated multi-seed submission with BOTH ratios
(entertainment_ratio, workstudy_ratio) added to all 3 models
(exp040/041/042). New ensemble CV = 0.96542 (+0.00031 over the previous
best submission's basis, exp036's 0.96511).

Same proven approach as exp025/037: 3 seeds, small 5% stratified holdout
per seed for early-stopping determination, blend with exp042's
re-optimized weights (lgbm=0.122, catboost=0.243, xgboost=0.635).
"""
import time
import json
import numpy as np
...
test["entertainment_ratio"] = (test["social_media_hours"] + test["gaming_hours"]) / test["daily_screen_time_hours"]
train["workstudy_ratio"] = train["work_study_hours"] / train["daily_screen_time_hours"]
test["workstudy_ratio"] = test["work_study_hours"] / test["daily_screen_time_hours"]
...
# save immediately so a downstream error doesn't cost the expensive training again
np.save("artifacts/pred_test_exp043_lgbm.npy", pred_lgbm)
np.save("artifacts/pred_test_exp043_catboost.npy", pred_catboost)
np.save("artifacts/pred_test_exp043_xgboost.npy", pred_xgb)

with open("artifacts/exp042_ensemble_weights.json") as f:
    weights = json.load(f)
print(f"Blend weights (from exp042): {weights}")
...
# guard against floating-point overshoot at the [0,1] boundary (weights summing
# to e.g. 0.9999999999999999 combined with a near-1.0 prediction can produce
# 1.0000000000000002) -- clip rather than fail on a non-issue
...
old_sub = pd.read_csv("submission_v3.csv")
diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
print(f"\nAll Phase 15 sanity checks passed.")
print(f"submission_v4.csv written: {len(submission)} rows")
print(f"mean |change| vs submission_v3.csv (prev best): {diff.mean():.5f}")
print(f"correlation with submission_v3.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
