"""
Experiment 025 — Updated multi-seed submission using the tuned models
(exp022 XGBoost, exp023 CatBoost) plus untuned LightGBM (exp001), blended
with exp024's re-optimized weights (lgbm=0.135, catboost_v2=0.266,
xgboost_v2=0.600). New ensemble CV = 0.96494 (+0.00018 over exp013/exp021).

Each seed's full-data fit uses a small 5% stratified holdout purely to
find an early-stopping point (we didn't log per-fold best_iteration during
the HPO runs) -- kept small to preserve most of the benefit of training on
close to 100% of the data, which exp018's LB result showed matters.
"""
import time
import json
import numpy as np
...
Xfull, yfull = train[NUM_COLS], train[TARGET].values
Xte = test[NUM_COLS]
...
for seed in SEEDS:
    ...
    print("LightGBM (untuned, exp001 params)...")
    lgbm_params = dict(
    ...
    print("CatBoost (tuned, exp023 params)...")
    cb_params = dict(catboost_tuned, iterations=9000, loss_function="Logloss",
    ...
    print("XGBoost (tuned, exp022 params)...")
...
with open("artifacts/exp024_ensemble_weights.json") as f:
    weights = json.load(f)
print(f"Blend weights (from exp024): {weights}")

final_pred = (
    weights["lgbm"] * pred_lgbm
    + weights["catboost_v2"] * pred_catboost
    + weights["xgboost_v2"] * pred_xgb
)
...
old_sub = pd.read_csv("submission_multiseed.csv")
diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
print(f"\nAll Phase 15 sanity checks passed.")
print(f"submission_v2.csv written: {len(submission)} rows")
print(f"mean |change| vs submission_multiseed.csv: {diff.mean():.5f}")
print(f"max |change| vs submission_multiseed.csv: {diff.max():.5f}")
print(f"correlation with submission_multiseed.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
