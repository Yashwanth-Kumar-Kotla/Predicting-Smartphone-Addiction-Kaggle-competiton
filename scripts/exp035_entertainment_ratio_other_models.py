"""
Experiment 035 — Does exp034's entertainment_ratio gain (+0.00018 on
tuned XGBoost) generalize to LightGBM and CatBoost? If the mechanism is
real (3-way ratios harder for trees to reconstruct via splits than 2-way),
it should show up across tree families, not just XGBoost.
"""
import time
import json
import numpy as np
...
# ---- LightGBM (exp001 params, untuned) ----
print("=" * 70)
print("LightGBM + entertainment_ratio")
print("=" * 70)
lgbm_params = dict(
...
np.save("artifacts/oof_exp035_lgbm_entertainment_ratio.npy", oof_lgbm)

# ---- CatBoost (exp023 tuned params) ----
print(f"\n{'=' * 70}\nCatBoost (tuned) + entertainment_ratio\n{'=' * 70}")
...
np.save("artifacts/oof_exp035_catboost_entertainment_ratio.npy", oof_cb)

log_experiment({
    "exp_id": "exp035",
    "model": "LightGBM + CatBoost, both + entertainment_ratio",
    ...
    "notes": "checking whether exp034's XGBoost gain (+0.00018) generalizes across tree families",
    "conclusion": "TBD",
})
