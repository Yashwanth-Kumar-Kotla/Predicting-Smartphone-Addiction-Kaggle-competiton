...
def main():
    ...
    X_openfe, Xte_openfe = transform(train[NUM_COLS].copy(), test[NUM_COLS].copy(), openfe_features[:40], n_jobs=8)
    ...
    np.save("artifacts/pred_test_exp066_lgbm.npy", pred_lgbm)
    np.save("artifacts/pred_test_exp066_catboost.npy", pred_catboost)
    np.save("artifacts/pred_test_exp066_xgboost.npy", pred_xgb)

    with open("artifacts/exp065_ensemble_weights.json") as f:
        weights = json.load(f)
    print(f"Blend weights (from exp065): {weights}")
    ...
    old_sub = pd.read_csv("submission_v7.csv")
    diff = (submission["addicted_label"] - old_sub["addicted_label"]).abs()
    print(f"\nAll Phase 15 sanity checks passed.")
    print(f"submission_v8.csv written: {len(submission)} rows")
    print(f"mean |change| vs submission_v7.csv (prev best): {diff.mean():.5f}")
    print(f"correlation with submission_v7.csv: {submission['addicted_label'].corr(old_sub['addicted_label']):.6f}")
