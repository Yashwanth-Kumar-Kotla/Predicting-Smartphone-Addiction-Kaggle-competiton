"""
Experiment 080 — Rebuild ensemble with exp079's tuned LightGBM (0.96698)
replacing the default-params version (0.96653), combined with exp076's
tuned XGBoost (0.96732) and exp072's CatBoost (0.96659, still untuned).
"""
import json
import numpy as np
...
print("Individual OOF AUCs (consensus features, LightGBM + XGBoost tuned):")
...
print(f"vs exp076 ensemble (untuned LightGBM, 0.96743): delta = {best_auc - 0.96743:+.5f}")

np.save("artifacts/oof_exp080_ensemble_lgbm_tuned.npy", best_blend)
with open("artifacts/exp080_ensemble_weights.json", "w") as f:
    json.dump(dict(zip(names, best_w.tolist())), f, indent=2)

from common import log_experiment
log_experiment({
    "exp_id": "exp080",
    "model": "Ensemble (tuned-LGBM+CatBoost+tuned-XGBoost, consensus-9 features)",
    ...
    "runtime_sec": "~2",
    "notes": "ensemble with exp079's tuned LightGBM replacing default params",
    "conclusion": "TBD",
})
print("\nLogged exp080.")
