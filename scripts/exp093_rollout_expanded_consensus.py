"""
Experiment 093 — exp092 validated the expanded-consensus features (20 new
features from OpenFE discovery over NUM_COLS+artifact_cols, +0.00018,
every fold improved) on XGBoost alone. Rolling out to LightGBM and
CatBoost, rebuilding the ensemble.
"""
import time
import pickle
import json
...
def main():
    ...
    print(f"\nLightGBM + expanded OOF AUC: {auc_lgbm:.5f} (vs exp089 artifacts-only 0.96726)")
    print(f"CatBoost + expanded OOF AUC: {auc_catboost:.5f} (vs exp089 artifacts-only 0.96686)")

    np.save("artifacts/oof_exp093_lgbm_expanded.npy", oof_lgbm)
    np.save("artifacts/oof_exp093_catboost_expanded.npy", oof_catboost)

    oof_xgb = np.load("artifacts/oof_exp092_expanded.npy")
    ...
    print(f"vs exp089 ensemble (artifacts-only, 0.96771): delta = {best_auc - 0.96771:+.5f}")

    np.save("artifacts/oof_exp093_ensemble_expanded.npy", best_blend)
    with open("artifacts/exp093_ensemble_weights.json", "w") as f:
        json.dump(dict(zip(names, best_w.tolist())), f, indent=2)

    from common import log_experiment
    log_experiment({
        "exp_id": "exp093",
        "model": "Ensemble (LGBM+CatBoost+XGBoost, all tuned, expanded consensus features)",
        "features": f"{len(feat_cols)} features (base+ratios+consensus9+artifacts+20 new expanded-consensus), all 3 models",
        "preprocessing": "exp091 3-seed consensus discovery over NUM_COLS+artifact_cols",
        ...
        "notes": f"lgbm={auc_lgbm:.5f} catboost={auc_catboost:.5f} xgboost=0.96772(exp092)",
        "conclusion": "TBD",
    })
    print("\nLogged exp093.")
